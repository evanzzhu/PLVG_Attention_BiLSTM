...
logging.basicConfig(filename='log/BILSTM_parameter_none_8_10_dh_step1_0103.log', level=logging.INFO)

df = pd.read_csv('data/load_data_train_full_all_8_10_d.csv', encoding='gb2312')
df_test = pd.read_csv('data/load_data_test_full_all_8_10_d.csv', encoding='gb2312')
...
def load_data(file_name):
    ...
    date_column = df['DATE']  # 保存 'DATE' 列
    df.drop('DATE', axis=1, inplace=True)  # 移除 'DATE' 列
    # 填充缺失值
    df.fillna(df.mean(), inplace=True)
    # 重新添加 'DATE' 列
...
centers = (centers - MIN_LOAD) / (MAX_LOAD - MIN_LOAD)
print(centers)
    ...
    for i in range(0, len(data) - day_len - num, num):
        ...
        for j in range(i, i + day_len):
            ...
            for c in range(2, 5):
                x.append(data[j][c])
            if j <= 7:
                x.append(centers[int(data[day_len + 1][6]) - 1][j % 8])
            elif 8 <= j <= 15:
                x.append(centers[int(data[day_len + 9][6]) - 1][j % 8])
            else:
                x.append(centers[int(data[day_len + 17][6]) - 1][j % 8])
            train_seq.append(x)
...
Dtr, train_date = nn_seq("data/load_data_train_full_all_8_10_d.csv", 32, 24)
Dte, test_date = nn_seq("data/load_data_test_full_all_8_10_d.csv", 32, 24)
# print(Dtr[:20])
loss_fn = nn.MSELoss(reduction='mean')
loss_fn = loss_fn.to(device)
seq_model = BiLSTM(6, 60, 1, 24, 32, 60)
seq_model = seq_model.to(device)
seq_model.load_state_dict(torch.load("model/PLVG_parameter_none_8_10_dh_step1.pkl"))
seq_model.eval()
total_test_loss = 0
total_accuracy = 0
rmse_list = []
act_build_energy = []
seq_pre_build_energy = []
lstm_pre_build_energy = []
bilstm_pre_build_energy = []
rnn_pre_build_energy = []
flag = True
with torch.no_grad():
    for data, target in Dte:
        if torch.cuda.is_available():
            data = data.cuda()
            target = target.cuda()
        seq_outputs = seq_model(data)
        seq_loss = loss_fn(seq_outputs, target)
        total_test_loss += seq_loss.item()
        prediction = seq_outputs.detach().cpu().numpy()
        ...
        if flag:
            ...
            seq_pre_build_energy = seq_outputs.reshape(-1)
            flag = False
        else:
            act_build_energy = torch.cat([act_build_energy, target.reshape(-1)], 0)
            seq_pre_build_energy = torch.cat([seq_pre_build_energy, seq_outputs.reshape(-1)], 0)
            # print(act_build_energy.shape)
            # print(pre_build_energy.shape)
        # pre_build_energy.append(target.reshape(-1))
        # accuracy = (outputs.argmax(1) == target).sum()
        # total_accuracy += accuracy
# print(f'整体测试集上的正确率:{total_accuracy / len(Dte)}')
mae, rmse, mape, r_2 = evaluation(act_build_energy.cpu().numpy(), seq_pre_build_energy.cpu().numpy())
print(f"mae:{mae},rmse:{rmse * (MAX_LOAD - MIN_LOAD)},mape:{mape},r_2:{r_2}")

e_csv_0 = csv.writer(open("result/result_PLVG.csv", 'w', encoding='gb2312', newline=''))
count = 0
e_csv_0.writerow(['Raw_Load', 'Pre_PLVG'])
tmp_24_act = []
tmp_24_pre = []
rmse1 = 0
for i in act_build_energy:
    tmp = []
    tmp.append(act_build_energy[count].item() * (MAX_LOAD - MIN_LOAD) + MIN_LOAD)
    tmp.append(seq_pre_build_energy[count].item() * (MAX_LOAD - MIN_LOAD) + MIN_LOAD)
    # tmp_24_act.append(act_build_energy[count].item() * (MAX_LOAD - MIN_LOAD) + MIN_LOAD)
    # tmp_24_pre.append(seq_pre_build_energy[count].item() * (MAX_LOAD - MIN_LOAD) + MIN_LOAD)
    count += 1
    # if count % 24 == 0:
    #     rmse1 = np.sqrt(mean_squared_error(tmp_24_act, tmp_24_pre))
    #     tmp.append(rmse1)
    #     tmp_24_act = []
    #     tmp_24_pre = []
    e_csv_0.writerow(tmp)